...
while 1:
    ...
    if ord(tmp) == 123:
		...
		if ord(tmp[0]) == 0:
			...
			water2_temp = int(ord(tmp[17]))
			print "Rpm: {0:5d} Speed: {1:3d} Vand temp: {2:3d} Luft temp: {3:3d} Batt: {4:2d} Pot: {5:3d} Map: {6:4d} Lambda: {7:4d}, Vand 2: {8:3d}".format(int((ord(tmp[1])<<8)+ord(tmp[2])*0.9408), speed, temp, air, batt, pot, Map, Lambda, water2_temp)
			# Datastreng der skal gemmes
			streng = '{0:5d},{1:3d},{2:3d},{3:3d},{4:2d},{5:3d},{6:4d},{7:4d},{8:3d}\n'.format(int((ord(tmp[1])<<8)+ord(tmp[2])*0.9408), speed, temp, air, batt, pot, Map, Lambda, water2_temp)
			f.write(streng)
		elif ord(tmp[0]) == 1:
